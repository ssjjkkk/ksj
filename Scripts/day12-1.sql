...
INSERT INTO emp (empno, ename, job, mgr) VALUES (7777, 'KSJ', 'KING', 7698);
...
9999번 사원번호를 갖는 사원의 deptno와 job 컬럼의 값을 SMITH사원의 deptno와 job 값으로 업데이트
...
==> UPDATE 쿼리1 실행할 때 안쪽 SELECT 쿼리가 2개가 포함됨 ==> 비효율적
	고정된 값을 업데이트 하는게 아니라 다른 테이블에 있는 값을 업데이트 할 때 비효율이 존재
...
DML(DATA Manipulation[조작] Languege) - 데이터를 다루는 SQL
SELECT, INSERTM UPDATE, DELETE

DDL(DATA Definition[정의] Languege) - 데이터를 정의하는 SQL
...
2번 방법으로 제약조건 생성 - 테이블 생성시 컬럼기술 이후 별도로 제약조건을 기술하는 방법
...
[ FOREIGN KEY - 참조키 ]
한 테이블의 컬럼의 값이 참조하는 테이블의 컬럼 값 중에 존재하는 값만 입력되도록 제어하는 제약조건
...
dept_test 테이블에 존재하지 않는 부서번호로 emp_test 테이블에 입력하는 경우;
