...
CREATE OR REPLACE FUNCTION getEmpName (p_empno emp.empno%TYPE) RETURN emp.ename%TYPE IS
...
CREATE OR REPLACE FUNCTION getdeptname (p_deptno dept.deptno%TYPE) RETURN dept.dname%TYPE IS
...
SELECT getdeptname(:asdf)
  FROM dual;

FUNCTION : fact
파라미터 : 0보다 큰 임의의 정수
리턴값 : 파라미터!(팩토리얼)값 ex) 5! = 5 * 4 * 3 * 2 * 1

CREATE OR REPLACE FUNCTION fact (p_num NUMBER) RETURN NUMBER IS
	n NUMBER;
BEGIN
	IF p_num = 0 THEN
		n := 1;
...
시나리오 : users 테이블에 pass 컬럼(비밀번호)이 존재
...
SELECT ename
  FROM emp
 WHERE empno = -99999;
 NO_DATA_FOUND ==> emp 테이블에 해당 empno 값을 넣을 수 없는 상태에서 조회가 되지 않았을 때
