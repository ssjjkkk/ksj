...
2. RECORD TYPE - 사용자 정의 타입
	행의 정보를 담을 수 있는 것은 ROWTYPE 과 동일, 사용자가 원하는 컬럼에 대해서만 정의
3. TABLE TYPE - 복수계의 행을 담을 수 있는 타입
...
dept 테이블의 10번 부서의 부서번호, 부서이름을 RECORD TYPE 으로 생성된 변수에 값을 담아서 출력
(dept 모든 컬럼을 조회하는것이 아니라, 일부 컬럼만 조회)
...
emp 테이블에서 사원번호, 사원이름, 사원부서, 업무를 RECORD TYPE 을 통해 출력하는 익명 PL/SQL 블럭을 작성하세요
('SMITH'의)
...
BEGIN
	SELECT empno, ename, deptno, job INTO v_emp_rec
	  FROM emp
	 WHERE ename = 'SMITH';
	DBMS_OUTPUT.PUT_LINE('empno : ' || v_emp_rec.empno || ', ename : ' || v_emp_rec.ename
						|| ', deptno : ' || v_emp_rec.deptno || ', job : ' || v_emp_rec.job);
END;
/


TABLE TYPE : 여러 건의 행을 저장할 수 있는 타입
...
CURSOR : SELECT 문이 실행되는 메모리 상의 공간
		 다량의 데이터를 변수에 담게 되면 메모리 낭비가 심해져 프로그램이 정상동작을 못할 수도 있음.
		 한 번에 모든 데이터를 인출하지 않고, 개발자가 직접 인출 단계를 제어 함으로써
		 데이터를 담지 않고도 개발하는 것이 가능

CURSOR 의 종류
묵시적 커서 : 커서 이름을 별도로 지정하지 않을 경우 ==> ORACLE 이 알아서 처리
...
속성
1. 커서명%FOUND : 해당 SQL 문에 의해 반환된 총 행수가 1개 이상일 경우 TRUE
2. 커서명%NOTFOUND : 해당 SQL 문에 의해 반환된 총 행수가 없을 경우 TRUE
...
CURSOR 의 경우는 반복문과 사용되는 일이 많기 때문에 PL/SQL 에서는 FOR LOOP 와 함께 사용하는 문법을 지원한다.
...
emp 테이블에서 특정 부서에 속하는 사원의 사번과 이름을 출력하는 로직을 파라미터가 있는 커서를 활용하여 작성해주세요.;
DECLARE
...
BEGIN
	...
	FOR emp_row IN emp_cur (:deptno) LOOP
