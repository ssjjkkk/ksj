...
--** IN, NOT IN 이용시 NULL 값의 존재 유무에 다라 원하지 않는 결과가 나올 수 있다.
--
...
SELECT empno, ename, deptno, (SELECT dname FROM dept WHERE deptno = e.DEPTNO) dname
  FROM EMP e  
;

-- 제품ID가 100 혹은 400 이면서 cid = 1인 시음주기를 갖는 제품정보를 조회하세요.
SELECT *
  FROM PRODUCT
...
--EXISTS : 조건에 해당하는 row의 존재 유무 체크 후 더 이상 수행하지 않음.
--		 메인쿼리의 결과값을 서브쿼리에 대입하여 조건 비교 후 결과를 출력한다.(메인쿼리 -> EXISTS 쿼리)
...
SELECT empno, ename
  FROM emp
 WHERE empno IN (7566, 7698)
 ORDER BY empno		-- ORDER BY 는 마지막에 써준다.
;
...
WITH temp AS (
		SELECT emp.DEPTNO d, dept.DEPTNO dd
		  FROM emp, dept
	)
SELECT *
  FROM temp
