...
실 사용 예 - 1. 실제 사용할 테이블과 별개로 보조 테이블에도 동일한 데이터 쌓기
		  ...
		  ==> 오라클 PARTITION 을 통해 좀더 효과적으로 관리 가능(정식버전에만 있음)
...
INSERT ALL
	...
	INTO emp_test2 (empno) VALUES (empno)	-- 첫번째 empno는 데이터를 입력할 테이블의 컬럼 이름
											-- 두번째 empno는 넣을 값.
SELECT 9999 empno, 'brown' ename FROM dual
 UNION all
SELECT 9998 empno, 'sally' ename FROM dual;
...
2. 테이블의 데이터를 이용하여 다른 테이블의 데이터를 업데이트 OR INSERT 하고 싶을 때
	일반 UPDATE 구문에서는 비효율 존재
	ALLEN 의 JOB과 DEPTNO 를 SMITH 사원과 동일하게 업데이트 하시오
...
--		WHERE ename = 'ALLEN';
	...
	ex - empno 9999, ename 'brown'
	 emp 테이블에 동일한 empno가 있으면 ename 을 업데이트 하고 싶고
...
3.
INSERT INTO emp (empno, ename) VALUES (9999, 'brwon');
...
emp - 16건, emp_test 2건 (7369, 9999 ==> emp 테이블에도 존재)
...
* 머지 조건 - 부서번호가 같은 데이터
동일한 부서가 있을 때 - 기존 loc 컬럼의 값 + _m 으로 업데이트
...
USING dual
   ON (a.deptno = :deptno)
 WHEN MATCHED THEN
 	UPDATE SET a.dname = :dname,
 			   a.loc = :loc
 WHEN NOT MATCHED THEN
 	INSERT (a.deptno, a.dname, a.loc) VALUES (:deptno, :dname, :loc); 
...
DELETE FROM emp_test WHERE empno = '9999'; 
DELETE FROM emp_test2 WHERE empno = '9998'; 
DELETE FROM emp_test2 WHERE empno = '9999'; 
