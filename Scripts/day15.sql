...
SELECT DECODE(GROUPING(job), 1, '총계', job) job, deptno, GROUPING(job),
...
SELECT DECODE(GROUPING(job), 1, '총', job) job,
	   DECODE(GROUPING_ID(job, deptno), 1, '소계', 3, '계', deptno) deptno,
	   GROUPING(job), GROUPING(deptno), SUM(sal + nvl(comm, 0)) sal
  FROM EMP
 GROUP BY ROLLUP (job, deptno);

SELECT DECODE(GROUPING(job), 1, '총', job) job,
	   DECODE(GROUPING(job) + GROUPING(deptno), 1, '소계', 2, '계', deptno) deptno,
	   GROUPING(job), GROUPING(deptno), SUM(sal + nvl(comm, 0)) sal
  FROM EMP
 GROUP BY ROLLUP (job, deptno);
