...
--	==> KING의 데이터는 조회되지 않음(총 15건의 데이터 중 13건의 데이터만 조인 성공)
--	
...
--OUTER JOIN : 조인 조건을 만족하지 않더라고 (조인에 실패하더라도) 기준이 되는 테이블 쪽의 데이터(컬럼)은 조회가
--			 되도록 하는 조인 방식
--			 
--OUTER JOIN :
...
SELECT e.EMPNO, e.ENAME, e.mgr, e2.ENAME
  FROM EMP e, EMP e2 
 WHERE e.MGR = e2.EMPNO(+)  -- 기준이 emp e 이므로 조회했을 때 안나오는 즉, NULL로 채워지는 emp e2쪽에
 ;							-- (+)를 붙여야 한다.
