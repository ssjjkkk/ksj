...
SELECT empno, ename, deptno, sal,
	   ...
	   SUM(sal) OVER (ORDER BY sal RANGE UNBOUNDED PRECEDING) c_sum2,
	   SUM(sal) OVER (ORDER BY sal DESC RANGE BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) c_sum3
  FROM emp
 ORDER BY sal
;
...
PL/SQL 프로그램의 종류는 크게 Procedual, FUNCTION, TRIGGER 로 나뉘어준다.
...
-- PL/SQL 에서는 한문장이 종료될때마다 ;를 사용하여 한문장이 끝난다는것을 명시
...
-- 실습2
CREATE OR REPLACE PROCEDURE insert_dept_test IS	
BEGIN
	INSERT INTO dept_test VALUES (99, 'dw', 'daejeon');
END;
/

CREATE OR REPLACE PROCEDURE insert_dept_test (p_deptno IN dept_test.deptno%TYPE,
...
BEGIN
	insert_dept_test(99, 'dw', 'daejeon');
END;
/
...
-- 실습3
CREATE OR REPLACE PROCEDURE update_dept_test IS
BEGIN
	UPDATE dept_test SET dname = 'dw_m' WHERE dname = 'dw';
END;
/

CREATE OR REPLACE PROCEDURE update_dept_test (p_dname1 IN dept_test.dname%TYPE,
											  p_dname2 IN dept_test.dname%TYPE) IS
BEGIN
	UPDATE dept_test SET dname = p_dname1 WHERE dname = p_dname2;
END;
/

BEGIN
	update_dept_test('dw_m', 'dw');
END;
/
