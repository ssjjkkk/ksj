...
-- ROWNUM 과 ORDER BY
-- SELECT SQL 의 실행 순서 : FROM -> WHERE -> SELECT -> ORDER BY

SELECT ROWNUM, empno, ename
  FROM emp
 ORDER BY ENAME
;
-- SELECT 에서 ROWNUM이 먼저 적용된 상태에서 ORDER BY 절이 실행되면서 뒤죽박죽
...
-- SELECT 절에 * 만 단독으로 사용하지 않고 콤마를 통해 다른 임의 컬럼이나 expression 을 표기한 경우
...
SELECT ROWNUM, a.*
  FROM (SELECT empno, ename
   		 ...
   		 ORDER BY ename) a;
;
-- ==> 실행순서 : 바깥족 FROM => 안쪽 FROM => 안족 SELECT => 안쪽 ORDER BY => 바깥쪽 SELECT
...
SELECT CONCAT(CONCAT('Hello', ',') , ' world!!') concat  -- Hello , World
	 , SUBSTR('Hello, World', 8, 5) substr
	 , LENGTH('Hello, World') LENGTH
	 , INSTR('Hello, World', 'o') instr1
	 , INSTR('Hello, World', 'o', 2) instr2
	 , INSTR('Hello, World', 'o', 1, 2) instr3
	 , LPAD('Hello, World', 15, '@') lpad
	 , RPAD('Hello, World', 17, '!') rpad -- => Hello, World!!!!!
	 , REPLACE('Hello, World', 'o', 'a') REPLACE
...
-- emp테이블에서 사원 이름이 smith인 사람을 조회하세요.
SELECT *
  FROM EMP
...
-- 두번째 쿼리는 14번을 실행해 봐야 소문자로 바뀌고 나서 비교를 함
