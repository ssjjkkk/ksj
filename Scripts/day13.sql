...
[ CHECK 제약 조건 - 컬럼에 입력되는 값을 검증하는 제약 조건 ]
	(ex - salary 컬럼(급여) 이 음수가 입력되는 것은 부자연스러움
		  ...
		  직원 구분이 정직원, 계약직 2개가 존재할 때 다른값이 들어오면 논리적으로 어긋남)
...
7. 제약조건을 활성화 / 비활성화 (ENABLE / DISABLE);
...
ksj 계정에게 VIEW 를 생성할 수 있는 권한 부여;
GRANT CREATE VIEW TO ksj;	-- system 계정에서 실행 해야함.
							-- 권한 부여 후 위 쿼리를 다시 실행하면 view를 생성할 수 있다.
...
[ hr 계정으로 접속하여 테스트]
v_emp VIEW 는 ksj 계정이 hr 계정에게 SELECT 권한을 주었기 때문에 정상적으로 조회 가능;
SELECT *
  FROM ksj.v_emp;
...
1. emp테이블에 신규 사원을 입력(기존 15건, 추가돼서 16건);
	INSERT INTO emp (empno, ename) VALUES (9999, 'james');
2. SELECT *
	 FROM v_emp;
	결과? ==> 16건
...
VIEW 는 SQL 이기 때문에 조인된 결과나, 그룹함수를 적용하여 행의 건수가 달라지는 SQL 도 VIEW 로 생성하는 것이 가능
...
INSERT int emp (empno) VALUES (seq_emp.nextVal);
